...
from cuda.core.experimental._resource_handles cimport ContextH, create_context_handle
...
cdef class Context:
    ...
    def _from_ctx(cls, handle: driver.CUcontext, int device_id):
        ...
        # Convert Python CUcontext to C-level CUcontext and create ContextH
        cdef cydriver.CUcontext c_ctx = <cydriver.CUcontext><uintptr_t>int(handle)
        ctx._handle = create_context_handle(c_ctx)
    ...
    def handle(self):
        ...
        cdef const cydriver.CUcontext* ptr = self._handle.get()
        if ptr != NULL:
    ...
    def __eq__(self, other):
        ...
        cdef const cydriver.CUcontext* ptr1 = self._handle.get()
        cdef const cydriver.CUcontext* ptr2 = _other._handle.get()
